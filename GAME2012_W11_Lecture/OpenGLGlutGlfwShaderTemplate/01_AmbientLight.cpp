...
// Texture variables.
GLuint blankID, wallTextureID, pillerTextureID;
...
void init(void)
{
	...
	unsigned char* walltexture = stbi_load("medieva_lBlocks_Brown.png", &width, &height, &bitDepth, 0);
	unsigned char* pillertexture = stbi_load("PillerTexture.jpg", &width, &height, &bitDepth, 0);
	...
	if (!walltexture) { cout << "Unable to load file!" << endl; }
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, walltexture);
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glGenerateMipmap(GL_TEXTURE_2D);
	stbi_image_free(walltexture);


	


	if (!pillertexture) { cout << "Unable to load file!" << endl; }
	
	glGenTextures(1, &pillerTextureID);

	glBindTexture(GL_TEXTURE_2D, pillerTextureID);


	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, pillertexture);
...
void display(void)
{
	...
	// Grid.
	transformObject(glm::vec3(1.0f, 1.0f, 1.0f), X_AXIS, -90.0f, glm::vec3(0.0f, 0.0f, 25.0f));
	...
	// cube map needs work 
	// floor 
		transformObject(glm::vec3(25.0f, 25.0f, 0.5f), X_AXIS, 90.0f, glm::vec3(0.0f, 0.0f, 0.0f));
		g_cube.DrawShape(GL_TRIANGLES);
	// left 	
		transformObject(glm::vec3(1.0f, 5.0f, 0.5f), X_AXIS, 0.0f, glm::vec3(0.0f, 0.0f, 0.0f));
		g_cube.DrawShape(GL_TRIANGLES);
	// right 
		transformObject(glm::vec3(1.0f, 5.0f, 0.5f), X_AXIS, 0.0f, glm::vec3(0.0f, 0.0f, 25.0f));
		g_cube.DrawShape(GL_TRIANGLES);
	// front 	
		transformObject(glm::vec3(1.0f, 5.0f, 0.5f), Y_AXIS, 90.0f, glm::vec3(0.0f, 0.0f, 25.0f));
		g_cube.DrawShape(GL_TRIANGLES);
	// back 
		transformObject(glm::vec3(1.0f, 5.0f, 0.5f), Y_AXIS, 90.0f, glm::vec3(25.0f, 0.0f, 25.0f));
		g_cube.DrawShape(GL_TRIANGLES);
...
void clean()
{
	...
	glDeleteTextures(1, &pillerTextureID);
	//glDeleteTextures(1, &wallTextureID);
}
